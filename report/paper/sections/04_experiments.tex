...
\begin{enumerate}
    \item \textbf{Main comparison.} Side-by-side PSNR/SSIM/$\ell_1$ means for zero-fill, real U-Net, and complex U-Net on the validation subset (Table~\ref{tab:main_results}).
    ...
    \item \textbf{Qualitative slices.} Consistent slice indices for all models so visual differences are attributable to the architecture rather than anatomy (Figure~\ref{fig:qualitative}).
    \item \textbf{Mask sweep.} Zero-fill grids for multiple accelerations/ACS settings (Figure~\ref{fig:zerofill_grid}) to contextualize the $R=6$ choice.
\end{enumerate}
